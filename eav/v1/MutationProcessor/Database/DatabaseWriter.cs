...
namespace MutationProcessor.Database
{
    public class DatabaseWriter : IDatabaseWriter
    {
        ...
        {
            var database = _client.GetDatabase("entities");
            var collection = database.GetCollection<Entity>(change.TenantId.ToString());
            ...
            try
            {
                ...
                if (change.EntityTemplateId == (int)EntityType.Contract)
                {
                    var entityUpdater = new EntityUpdater(change, collection, cancellationToken);
                    await entityUpdater.AddOrUpdateChildEntity();
                    return true;
                }
            ...
            {
                _logger.LogError(ex, "Something went wrong inserting the change");
                return false;
            }
        ...
        private static async Task<UpdateResult> EnsureEntity(Change change,
        ...
        private static async Task<UpdateResult> AddMutationIfNotExist(Change change, IMongoCollection<Entity> collection, CancellationToken cancellationToken)
        {
            ...
            return await collection.UpdateOneAsync(mutationFilter, update, new UpdateOptions(), cancellationToken);
        }
        ...
        private static async Task UpdateMutation(Change change, IMongoCollection<Entity> collection, CancellationToken cancellationToken)
        {
            ...
            await collection.UpdateOneAsync(mutationFilter, update, new UpdateOptions(), cancellationToken);
        }
    }
}